# MIRNext
  * MIRNext is an independently maintained continuation fork of the
    [MIR Project](https://github.com/vnmakarov/mir), originally created by
    Vladimir Makarov.
  * MIRNext maintains both interpreter and JIT/generator execution, with
    actively maintained x86_64 and aarch64 support.
  * ppc64le, s390x, and riscv64 are no longer maintained targets for MIRNext.
  * MIRNext is not affiliated with, sponsored by, or endorsed by the original
    MIR Project maintainers.
  * MIR means **M**edium **I**nternal **R**epresentation
  * MIRNext's goal is to provide a compact basis for embeddable interpreters
    and lightweight JITs, treating both execution modes as first-class.
...
## Disclaimer
   * **There is absolutely no warranty that the code will work for any tests except ones given here and on platforms
     other than x86_64 Linux/OSX/Windows and aarch64 Linux/OSX(Apple Silicon)**

## MIR
  * MIR is strongly typed IR
  * MIR can represent machine 32-bit and 64-bit insns of different architectures
  * [MIR.md](MIR.md) contains detail description of MIR and its API.
...
## The current state of MIRNext
  ...
  * MIRNext no longer includes the unfinished LLVM IR to MIR translator.

## The possible future state of MIRNext
 ...
 * Files `mir-gen.h`, `mir-gen.c`, `mir-gen-x86_64.c`, and `mir-gen-aarch64.c`
   contain the maintained code for MIR JIT compiler
   * Files `mir-gen-x86_64.c` and `mir-gen-aarch64.c` are the maintained
     machine dependent code of JIT compiler
 ...
 * Files `c2mir/c2mir.h`, `c2mir/c2mir.c`, `c2mir/c2mir-driver.c`, and `c2mir/mirc.h` contain code for
   C to MIR compiler.  Files in directories `c2mir/x86_64` and `c2mir/aarch64`
   contain the maintained machine-dependent code for C to MIR compiler
...
## Playing with current MIRNext project code
...
## Porting MIRNext
  * MIRNext maintains x86_64 and aarch64 targets on Linux and MacOS, with
    x86_64 Windows support inherited from MIR.
  * [HOW-TO-PORT-MIR.md](HOW-TO-PORT-MIR.md) outlines process of porting MIRNext
    * On my estimation an experienced developer can port MIRNext (including `c2m`) to another target for 1-2 months
