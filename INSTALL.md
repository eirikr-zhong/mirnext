...
You can build MIRNext in source directory simply with `make` or `make
all`

You can also build MIRNext in a separate directory.  In this case you
need to use `make SRC_DIR=<path to MIRNext sources> -f <path to MIRNext sources>/GNUmakefile`.
**All other calls of make** should have the
same `SRC_DIR` value and `-f` argument on the make command line.

 By default MIRNext is built in release mode (with optimizations).  If you
...
# Testing and Benchmarking MIRNext

 Please use `make test` or `make bench` for testing and benchmarking
with the right `SRC_DIR` if you build MIRNext in a different directory.
